...
function buildEnemy(zone, enemyDef) {
  // PVP: enemyDef already has full stats, zone is null
  if (!zone) {
    return {
      name:   enemyDef.name   || 'Player',
      emoji:  enemyDef.emoji  || '👤',
      hp:     enemyDef.hp     || 100,
      maxHp:  enemyDef.hp     || 100,
      atk:    enemyDef.atk    || 20,
      reward: enemyDef.reward || 50,
      isPVP:  true,
    };
  }
...
export function openCombat(zone, enemyDef) {
  ...
  // FIX: guard zone.name for PVP (zone is null)
  document.getElementById('combat-zone-label').textContent = zone ? zone.name : 'PVP Battle';
...
function renderEnemy() {
  ...
  // FIX: guard CS.zone for PVP
  document.getElementById('combat-enemy-type').textContent    = CS.zone ? CS.zone.type : 'PVP';
...
function combatVictory() {
  ...
  // FIX: Level up — subtract threshold so XP doesn't carry over incorrectly
  let levelText = '';
  if (rawNewXp >= xpToNext) {
    updates.level = currentLevel + 1;
    updates.xp    = rawNewXp - xpToNext; // carry over remainder
  ...
  // Zone clear — only for non-PVP
  let zoneCleared = false;
  if (zone && !e.isPVP) {
    updates.kills = newKills;
...
/* ══════════════════════════════════════
   ATMOSPHERIC CANVAS
   FIX: moved to functions — no top-level DOM access
══════════════════════════════════════ */
let cvs = null, ctx = null, animId = null, t = 0;

function startCanvas() {
  // FIX: get canvas reference here, not at module load time
  cvs = document.getElementById('combat-canvas');
  if (!cvs) return;
